...
#   actor_lr = 0.00025,
#   critic_lr = 0.001,
#   architecture = "A3C_PPO",
# )
#wandb.init(name='PPO1', project="real_environment", sync_tensorboard=True, config=config,)
#wandb.init(name='PPO1', project="reinforcement_test", sync_tensorboard=True)
...
class Actor_Model:
    def __init__(self, input_shape, action_space, lr, optimizer):
        self.Actor_name = '0106_Env04_Actor_continuous_model.h5'
        ...
        output_v = Dense(1, activation="sigmoid")(X)  # [0,1]
        output_w = Dense(1, activation="tanh")(X)     # [-1,1]
    ...
    def ppo_loss_continuous(self, y_true, y_pred):
        ...
        loss_entropy = ENTROPY_LOSS_RATIO * K.mean(-(K.log(2*np.pi*variance)+1) / 2)
        actor_loss = actor_loss + loss_entropy

        return actor_loss

    def gaussian_likelihood(self, actions, pred): # for keras custom loss
        log_std = -0.5 * np.ones(self.action_space, dtype=np.float32)    # covariance 
...
class Critic_Model:
    def __init__(self, input_shape, action_space, lr, optimizer):
        self.Critic_name= '0106_Env04_Critic_continuous_model.h5'
        X_input = Input(input_shape)
...
class PPOAgent:
    ...
    def __init__(self, env_name, model_name=""):
        ...
        self.replay_count = 0
        self.writer = SummaryWriter(comment="_"+self.env_name+"_"+self.optimizer.__name__+"_")
        
        # Instantiate plot memory
        self.scores_, self.episodes_, self.average_ = [], [], [] # used in matplotlib plots

        # Create Actor-Critic network models
        self.Actor = Actor_Model(input_shape=self.state_size, action_space = self.action_size, lr= self.actor_lr, optimizer = self.optimizer)
        self.Critic = Critic_Model(input_shape=self.state_size, action_space = self.action_size, lr=self.critic_lr, optimizer = self.optimizer)
        

        #self.Actor_name = 'Env04_Actor_continuous_model.h5'
        #self.Critic_name= 'Env04_Critic_continuous_model.h5'
        #self.load() # uncomment to continue training from old weights
        self.Actor_name = "Env04_Actor_continuous_model02.h5"
        self.Critic_name = "Env04_Critic_continuous_model02.h5"
    ...
    def act(self, state):
        ...
        #pred = np.array([[pred_v[0,0], pred_w[0,0]]])
        print('pred: ' + str(pred))
        ...
        #print('action before reparameterization: ' + str(action))
        ...
        wandb.log({"linear_velocity": v_action})
        wandb.log({"angular_velocity": w_action})
    ...
    def PlotModel(self, score, episode, save=True):
        ...
        if (self.average_[-1] >= self.max_average and save) or str(episode)[-3:] == "000":
            ...
            self.save()
            self.Actor.Actor.save('08_2_entropy_Env04_Actor_continuous_model.h5')
            self.Critic.Critic.save('08_2_entropy_Env04_Critic_continuous_model.h5')
            SAVING = "SAVING"
    ...
    def run_batch(self):
        ...
        print('action_space: ' + str(self.env.action_space))  
        print('action_size (the shape of action space ): ' + str(self.action_size))                                #self.env.action_space.shape[0]
        print('observation space: ' + str(self.env.action_space))
        print('state_size (the shape of observation space): ' + str(self.state_size))
    ...
    def test(self, test_episodes = 50):#evaluate
        self.load()
        for e in range(test_episodes):
            ...
            while not done:
                ...
                print("predict action ={}".format(action))

                # # re-parameterization trick 
                v_random = np.random.uniform(0, 1)
                w_random = np.random.uniform(-1, 1)
                action_random = np.array([[v_random, w_random]])
                action = action +  action_random * self.std
                v_action = np.clip(action[0,0], 0, 1)
                w_action = np.clip(action[0,1], -1, 1)
                action = np.array([[v_action, w_action]])   
                action = np.squeeze(action, axis = 0)
                print('after re-parameterization, action: ' + str(action))
                ...
                if done:
                    ...
                    break
        self.env.close()
        print("Current time {}, {}".format(now.secs, now.nsecs))

...
class Actor_Model:
    def __init__(self, input_shape, action_space, lr, optimizer):
        self.Actor_name = 'Models/MyTurtleBot2Wall-v0_PPO_Actor.h5'
...
class Critic_Model:
    def __init__(self, input_shape, action_space, lr, optimizer):
        self.Critic_name= 'Models/MyTurtleBot2Wall-v0_PPO_Critic.h5'
        X_input = Input(input_shape)
...
class PPOAgent:
    ...
    def __init__(self, env_name):
        ...
        self.EPISODES = 25000 # total episodes to train through all environments
        self.episode = 0 # used to track the episodes total count of episodes played through all thread environments
        self.max_average = 0 # when average score is above 0 model will be saved
        self.lr = 0.00025
        # self.lr = 1.829*math.pow(10,-14)
        ...
        self.Actor_name = "Models/{}_PPO_Actor.h5".format(self.env_name)
        self.Critic_name = "Models/{}_PPO_Critic.h5".format(self.env_name)
    ...
    def load(self):
        ...
        print(self.Actor.Actor.get_weights())
        self.Critic.Critic.load_weights(self.Critic_name)
        self.Critic.Critic.summary()
        print(self.Critic.Critic.get_weights())
    def save(self):
    ...
    def PlotModel(self, score, episode):
        ...
        if str(episode)[-2:] == "00":# much faster than episode % 100
            ...
            try:
                ...
                pass
        # saving best models
        if self.average_[-1] >= self.max_average or str(episode)[-3:] == "000":
    ...
    def run_batch(self): # train every self.Training_batch episodes
        state = self.env.reset()
        state = np.reshape(state, [1, self.state_size[0]])
        #state = np.expand_dims(state, axis=-1)   #test
        done, score, SAVING = False, 0, ''
        while True:
            # Instantiate or reset games memory
            states, next_states, actions, rewards, predictions, dones = [], [], [], [], [], []
            for t in range(self.Training_batch):
                #self.env.render()
                # print('state: ' + str(state))  #Hien thi gia tri Input
                value = self.Critic.predict(state)
                # Actor picks an action
                action, action_onehot, prediction = self.act(state)
                # Retrieve new state, reward, and whether the state is terminal
                next_state, reward, done, _ = self.env.step(action)
                #next_state = np.reshape(next_state, [1, self.state_size[0]]) #test
                ...
                states.append(state)
                next_states.append(np.reshape(next_state, [1, self.state_size[0]]))
                #next_states.append(np.expand_dims(next_state, axis=-1))  #test
                actions.append(action_onehot)
                rewards.append(reward)
                dones.append(done)
                predictions.append(prediction)
                ...
                if done:
                    average, SAVING = self.PlotModel(score, self.episode)
                    # wandb.log({"Average": average})
                    print("episode: {}/{}, score: {}, average: {:.2f}, learning_rate: {:.2f} {}".format(self.episode, self.EPISODES, score, average, self.lr, SAVING))
                    self.writer.add_scalar('Workers:{}/score_per_episode'.format(1), score, self.episode)
                    self.writer.add_scalar('Workers:{}/learning_rate'.format(1), self.lr, self.episode)
                    ...
                    self.epsilon_current_value=self.epsilon_current_value-(self.epsilon_initial_value-self.epsilon_final_value)/4000
                    self.temperature_parameter_current_value=self.temperature_parameter_current_value-(self.temperature_parameter_initial_value-self.temperature_parameter_final_value)/4000 
                    self.episode += 1
            self.replay(states, actions, rewards, predictions, dones, next_states)
            if self.episode >= self.EPISODES:
                break
        ...
        while self.episode < self.EPISODES:
            ...
            for worker_id, parent_conn in enumerate(parent_conns):
                ...
                if done:
                    ...
                    self.writer.add_scalar('Workers:{}/score_per_episode'.format(num_worker), score[worker_id], self.episode)
                    self.writer.add_scalar('Workers:{}/learning_rate'.format(num_worker), self.lr, self.episode)
                    score[worker_id] = 0
                    if(self.episode < self.EPISODES):
                        self.episode += 1
    ...
    def test(self, test_episodes = 1):
        ...
        for e in range(test_episodes):
            state = self.env.reset()
            # normalize state
            state = state - np.mean(state)    # normalizing the result
            state /= np.std(state)            # divide by standard deviation
            state = np.reshape(state, [1, self.state_size[0]])
            done = False
            score = 0
            while not done:
                ...
                score += reward
                if reward == 200.0:
                    print("Sucessful with score: {}".format(score))
                    i = 1
                elif reward == -300.0:
                    print("Unsucessful with score: {}".format(score))
                    i = 0
                if done:
                    #print("episode: {}/{}, score: {}".format(e + 1, test_episodes, score))
                    break
            s = s + i
            rate = (s/float(test_episodes))*100
        print(s)

...
from tensorflow.keras.layers import Input, Dense, Lambda, Add, Conv1D, Conv2D, Flatten
...
def OurModel(input_shape, action_space, lr):
    Actor_name = 'Models/MyTurtleBot2Wall-v0_APPO_0.0001_Actor.h5'
    Critic_name = 'Models/MyTurtleBot2Wall-v0_APPO_0.0001_Critic.h5'
    ...
    X = Conv1D(32, 9, strides=4, padding="same", activation="elu", data_format="channels_first", input_shape=input_shape)(X_input)
    X = Conv1D(64, 5, strides=2, padding="same", activation="elu", data_format="channels_first")(X)
    #X = Conv1D(64, 3, strides=1, padding="same", activation="elu", data_format="channels_first")(X)
    ...
    #Actor = load_model(Actor_name, custom_objects={'ppo_loss': ppo_loss})
    Actor.compile(loss=ppo_loss, optimizer=RMSprop(lr=lr))

    Critic = Model(inputs = X_input, outputs = value)
    #Critic = load_model(Critic_name)
...
class PPOAgent:
    ...
    def __init__(self, env_name):
        ...
        self.EPISODES, self.episode, self.max_average = 1000000, 0, 150.0 # specific for pong
        self.lock = Lock()
        self.lr = 0.0001
        ...
        self.state_size = (22,1)
        #self.state_size = (self.env.observation_space.shape,1)
        #print('state_size: ' + str(np.shape(self.state_size)))
        ...
        # Retraining Actor-Critic network model
    ...
    def act(self, state):
        ...
        action = np.random.choice(self.action_size, p=prediction)     #Exploitation

        #if x < self.epsilon_current_value:    #Exploration
            #q_values = ((self.Actor.predict(state)[0])**(1/self.temperature_parameter_current_value))
            #print('q_values: ' + str(q_values))
            #top_actions = q_values.argsort()[-self.nma:][::-1]
            #print('top_actions: ' + str(top_actions))
            #action = random.choice(top_actions)
            #print('action_exploration: ' + str(action))
        #else:
            #action = np.random.choice(self.action_size, p=prediction)     #Exploitation

            #action = np.argmax(self.Actor.predict(state))

            # q_values=(self.Actor.predict(state)) #Exploitation
            # print('q_values: ' + str(q_values))
            # max_Q = np.argmax(q_values)
            # print('max_Q: ' + str(max_Q))
            # action = max_Q

            #print('action_exploitation: ' + str(action))
    ...
    def replay(self, states, actions, rewards, predictions):
        ...
        # (dinh hinh lai bo nho thanh hinh dang thich hop de training)
        ...
        pylab.show()
        '''
        # stack everything to numpy array (xep chong moi thu vao mang numpy)
        y_true = np.hstack([advantages, predictions, actions])

        #print('lenght_reward: ' + str(len(rewards)))
        ...
        self.Actor.fit(states, y_true, epochs=self.EPOCHS, verbose=0, shuffle=True, batch_size=len(rewards),callbacks=[callback, WandbCallback()])
    ...
    def GetImage(self, frame, image_memory):
        if image_memory.shape == (1, 4, 320, 320):
            #print('image_memory: ' + str(image_memory.shape))
            image_memory = np.squeeze(image_memory)
            #print('image_memory: ' + str(np.shape(image_memory)))

        # croping frame to 80x80 size
        frame_cropped = frame[35:195:2, ::2,:]
        # print('frame: ' + str(np.shape(frame)))
        # print('frame_cropped: ' + str(np.shape(frame_cropped)))
        if frame_cropped.shape[0] != self.COLS or frame_cropped.shape[1] != self.ROWS:
            # OpenCV resize function 
            frame_cropped = cv2.resize(frame, (self.COLS, self.ROWS), interpolation=cv2.INTER_CUBIC)
            #print('frame_cropped_2: ' + str(np.shape(frame_cropped)))
        ...
        #return image_memory
        #return np.reshape(image_memory, [1,self.state_size])

    def GetLaserData(self, frame, image_memory):
        # frame = frame - np.mean(frame) # normalizing the result
        # frame /= np.std(frame) # divide by standard deviation
        frame_reshape = np.reshape(frame, (1, 22))
        return np.expand_dims(frame_reshape, axis=-1)
    ...
    def reset(self, env):
        ...
        #     state = self.GetLaserData(frame, image_memory)
    ...
    def step(self, action, env, image_memory):
        ...
        next_state = self.GetLaserData(next_state, image_memory)
    ...
    def run(self):
        for e in range(self.EPISODES):
            ...
            while not done:
                ...
                print('episode: ' + str(e) + ', action=' + str(action) + ', reward=' + str(reward) + ', q_value=' + str(value))
                ...
                if done:
                    ...
                    # saving best models
                    if average >= self.max_average or str(e)[-2:] == "00":
            ...
            self.epsilon_current_value = self.epsilon_current_value-(self.epsilon_initial_value-self.epsilon_final_value)/2000
            self.temperature_parameter_current_value = self.temperature_parameter_current_value-(self.temperature_parameter_initial_value-self.temperature_parameter_final_value)/2000   
    ...
    def test(self, Actor_name, Critic_name):
        ...
        self.EPISODES_TEST = 21
        for e in range(1, self.EPISODES_TEST):
            state = self.reset(self.env)
            done = False
            score = 0
            count = 0
            ...
            while not done:
                ...
                if done:
                    if score > 200.0:
                        print("Episode: {}/{}, Sucessful with score: {}".format(e, self.EPISODES_TEST-1, score))
                    else:
                        print("Episode: {}/{}, Unsucessful ".format(e, self.EPISODES_TEST-1))
                    break
        self.env.close()
...
if __name__ == "__main__":
    ...
    #agent.test('Pong-v0_APPO_0.0001_Actor_CNN.h5', '')
    #agent.test('Models/MyTurtleBot2Wall-v0_APPO_0.0001_Actor (3rd copy).h5', '')
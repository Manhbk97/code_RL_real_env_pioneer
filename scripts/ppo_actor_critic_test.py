...
from openai_ros.task_envs.turtlebot2 import turtlebot2_wall_test
...
class PPOAgent:
    ...
    def PlotModel(self, score, episode):
        ...
        if self.average_[-1] >= self.max_average or str(episode)[-3:] == "000":
            ...
            self.save()
            SAVING = "SAVING"
            # decreate learning rate every saved model
            self.lr *= 0.95
        ...
        # Test restart-learning rate
        # if str(episode)[-3:] == "000":
        #     self.lr = 0.00025
        #     print('lr: ' + str(self.lr))
        #     K.set_value(self.Actor.Actor.optimizer.learning_rate, self.lr)
        #     K.set_value(self.Critic.Critic.optimizer.learning_rate, self.lr)
        #     SAVING = ""
        # elif self.average_[-1] >= self.max_average or str(episode)[-2:] == "00":
        #     self.max_average = self.average_[-1]
        #     self.save()
        #     SAVING = "SAVING"
        #     # decreate learning rate every saved model
    ...
    def run_batch(self): # train every self.Training_batch episodes
        ...
        while True:
            ...
            for t in range(self.Training_batch):
                ...
                if done:
                    average, SAVING = self.PlotModel(score, self.episode)
                    print("episode: {}/{}, score: {}, average: {:.2f}, learning_rate: {:.7f} {}".format(self.episode, self.EPISODES, score, average, self.lr, SAVING))
                    self.writer.add_scalar('Workers:{}/score_per_episode'.format(1), score, self.episode)
                    self.writer.add_scalar('Workers:{}/learning_rate'.format(1), self.lr, self.episode)
                    ...
                    # if str(self.episode)[-4:] == "0000":
                    #     self.epsilon_current_value = 1.0
                    #     self.temperature_parameter_current_value = 5.0
                    # else:
                    #     self.epsilon_current_value=self.epsilon_current_value-(self.epsilon_initial_value-self.epsilon_final_value)/4000
                    #     self.temperature_parameter_current_value=self.temperature_parameter_current_value-(self.temperature_parameter_initial_value-self.temperature_parameter_final_value)/4000 
                    self.episode += 1

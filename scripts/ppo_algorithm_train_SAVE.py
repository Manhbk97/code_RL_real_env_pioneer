...
class PPOAgent:
    ...
    def __init__(self, env_name):
        ...
        self.EPISODES, self.episode, self.max_average = 10000, 0, 600.0 # specific for pong
        self.lock = Lock()
        self.lr = 0.0001
    ...
    def replay(self, states, actions, rewards, predictions):
        ...
        self.Actor.fit(states, y_true, epochs=self.EPOCHS, verbose=0, shuffle=True, batch_size=len(rewards), callbacks=[WandbCallback(),
        TensorBoard(log_dir=wandb.run.dir)])
        self.Critic.fit(states, discounted_r, epochs=self.EPOCHS, verbose=0, shuffle=True, batch_size=len(rewards),callbacks=[tensor_board])
